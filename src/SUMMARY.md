# Summary

- [Introduction](introduction.md)

# Part I - Fundamentals

- [Why Networking Matters](part1/why-networking.md)
- [How Computers Actually Communicate](part1/communication.md)
- [The TCP/IP Stack](part1/tcpip.md)
- [TCP, UDP, Ports and Sockets](part1/tcp-udp.md)
- [Routing](part1/routing.md)
- [ARP: Finding the Next Hop](part1/arp.md)
- [ICMP](part1/icmp.md)
- [How DNS Really Works](part1/dns.md)

# Part II - Linux Networking Basics

- [Linux Network Interfaces](part2/interfaces.md)
- [How Linux Makes Routing Decisions](part2/routing-linux.md)
- [Observing Network Traffic on Linux](part2/traffic.md)
- [Understanding Linux Network Connections](part2/connections.md)
- [Build a Small Linux Network](part2/lab-small-network.md)

# Part III - Virtual Networking

- [Why Linux Needs Virtual Networking]()
- [Network Namespaces: Multiple Networks on One Linux Host]()
- [Virtual Ethernet (veth): The Cable Inside Linux]()
- [Linux Bridges: Building a Software Switch]()
- [Building a Complete Virtual Network]()
- [Linux as a Router]()
- [VLANs and TUN/TAP: Two More Linux Networking Primitives]()

# Part IV - Packet Processing

- [Following a Packet Through the Linux Kernel]()
- [Netfilter: The Linux Packet Processing Framework]()
- [Connection Tracking: The Hidden State Machine]()
- [Network Address Translation (NAT)]()
- [iptables: Building Firewalls on Linux]()
- [nftables: Modern Linux Firewalling]()
- [Tracing Packet Flow]()

# Part V - Advanced Linux Networking

- [Why Traditional Networks Don't Scale]()
- [GRE: The Simplest Linux Tunnel]()
- [VXLAN: Extending Layer 2 Across Layer 3]()
- [Open vSwitch: Beyond Linux Bridge]()
- [Advanced Routing with Policy Routing and VRF]()
- [MTU, Fragmentation and Path MTU Discovery]()
- [Building a Multi-Host Virtual Network]()